...
const PostList = () => {
  const { posts, fetchPosts } = useContext(PostContext); // Assuming fetchPosts is a function in the context to fetch posts
  const [currentPage, setCurrentPage] = useState(1);
  const [searchTerm, setSearchTerm] = useState('');
  const [loading, setLoading] = useState(true);
  const postsPerPage = 4;
  ...
  useEffect(() => {
    const loadPosts = async () => {
      if (!posts || posts.length === 0) {
        await fetchPosts(); // Fetch posts if not already available
      } else {
        setLoading(false);
      }
    };
